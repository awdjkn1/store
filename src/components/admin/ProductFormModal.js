/* CONVERTED inline px→rem by scripts/convert-inline-px-to-rem.js on 2025-11-11T19:57:07.889Z */
...
const ProductFormModal = ({ token, show, onClose, onProductCreated }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.6)', zIndex: 2000 }}>
      <div style={{ width: 'min(56.25rem, 96%)', background: '#0f1724', borderRadius: 12, padding: 20, boxShadow: '0 0.5rem 2rem rgba(0,0,0,0.6)', color: '#fff', position: 'relative' }}>
        <button onClick={onClose} style={{ position: 'absolute', right: 12, top: 12, background: 'transparent', border: 'none', color: '#9ca3af', fontSize: 28, cursor: 'pointer' }} aria-label="Close">×</button>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Product Name" required style={{ padding: 10, borderRadius: 8, border: '0.0625rem solid #222', background: '#0b1220', color: '#fff' }} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <label style={{ color: '#cbd5e1' }}>Description</label>
              <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Short Description" rows={3} style={{ padding: 10, borderRadius: 8, border: '0.0625rem solid #222', background: '#0b1220', color: '#fff' }} />
            </div>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 13.75rem', display: 'flex', flexDirection: 'column', gap: 8 }}>
                <label style={{ color: '#cbd5e1' }}>Price ($)</label>
                <input value={price} onChange={e => setPrice(e.target.value)} placeholder="Price" type="number" min="0" step="0.01" inputMode="decimal" required style={{ padding: 10, borderRadius: 8, border: '0.0625rem solid #222', background: '#0b1220', color: '#fff' }} />
              </div>
              <div style={{ flex: '1 1 11.25rem', display: 'flex', flexDirection: 'column', gap: 8 }}>
                <label style={{ color: '#cbd5e1' }}>Lego Pieces</label>
                <input value={legoPieces} onChange={e => setLegoPieces(e.target.value)} placeholder="Pieces" type="number" min="0" step="1" required style={{ padding: 10, borderRadius: 8, border: '0.0625rem solid #222', background: '#0b1220', color: '#fff' }} />
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <input
                ...
                style={{ padding: 6, borderRadius: 8, border: '0.0625rem solid #222', background: '#0b1220', color: '#fff' }}
              />
              <small style={{ color: '#9ca3af' }}>{files.length ? `${files.length} file(s) selected` : 'You can upload up to 10 images.'}</small>
            </div>
            <button type="submit" disabled={uploading} style={{ width: '100%', marginTop: 8, fontSize: 18, padding: '0.75rem 1rem', borderRadius: 10, border: 'none', background: '#ff6b35', color: '#fff', fontWeight: 700, cursor: 'pointer', opacity: uploading ? 0.7 : 1 }}>
